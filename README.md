...
![The fixture picker opens, listing three prompts chosen for the shape of their
distributions: peaked, flat and bimodal. Then tokens stream into the completion
while the candidate panel redraws for each one. The temperature slider sweeps
from 0.7 to 2 and back down, flattening the distribution and then collapsing it
onto a single candidate; top-p tightens and the discarded candidates fade to
ghosts behind the survivors. A fork is taken from an alternative token and the
new line runs to the end.](docs/demo.gif)

<sub>The fixture picker, then the tour the **D** key runs. Sped up ~2.4×. Same
recording at full size and speed: [docs/demo.mp4](docs/demo.mp4).</sub>
